...
#endif
{
  ...
  while (svptest_first(pg, pg))
  {
    ...
    if (!svptest_any(pg, svnot_b_z(pg, pg_if))) {
      svuint64_t sv_if = svsub_n_u64_m(pg, akt_group, net->full_groups);
      sv_if = svmad_n_u64_m(pg, sv_if,
        svdup_u64(net->max_elems - 1),
        net->full_groups * net->max_elems);
      result = svadd_u64_m(pg, result, sv_if);
      goto latch;
    }
    // pg_if

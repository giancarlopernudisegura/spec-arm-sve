...
#endif
{
    ...
    for (i = 1; i <= net->n_trips; i++)
    {
        ...
        akt_group = _actArc % net->nr_group;
        if (akt_group > net->full_groups) {
            result = (_actArc / net->nr_group) + (net->full_groups * net->max_elems + (akt_group - net->full_groups) * (net->max_elems - 1));
        }
        else {
            result = (_actArc / net->nr_group) + (akt_group * net->max_elems);
        }
        arc = net->arcs + result;
        arc->cost =

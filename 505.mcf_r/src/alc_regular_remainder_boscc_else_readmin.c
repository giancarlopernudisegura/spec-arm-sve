...
#endif
{
  ...
  while (svptest_first(pg1, pg1))
  {
    ...
    if (!all(pg1, pg1_else) && !all(pg2, pg2_else) && svcntp_b64(pg1, pg1_else) + svcntp_b64(pg2, pg2_else) > svcntd()) {
      ...
      if (!svptest_any(pgR, svnot_b_z(pgR, pgR_else))) {
        svuint64_t sv_else = svmul_n_u64_m(pgR, akt_groupR, net->max_elems);
        resultR = svadd_u64_m(pgR, resultR, sv_else);
        goto LATCH4;
      }
      // pgR_if
    ...
    if (!svptest_any(pg1, svnot_b_z(pg1, pg1_else))) {
      svuint64_t sv_else = svmul_n_u64_m(pg1, akt_group1, net->max_elems);
      result1 = svadd_u64_m(pg1, result1, sv_else);
      goto LATCH2;
    }
    // pg1_if
    ...
    if (!svptest_any(pg2, svnot_b_z(pg2, pg2_else))) {
      svuint64_t sv_else = svmul_n_u64_m(pg2, akt_group2, net->max_elems);
      result2 = svadd_u64_m(pg2, result2, sv_else);
      goto LATCH3;
    }
    // pg2_if
